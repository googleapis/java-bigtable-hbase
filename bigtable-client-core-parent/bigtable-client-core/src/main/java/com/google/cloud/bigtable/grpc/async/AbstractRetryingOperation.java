...
public abstract class AbstractRetryingOperation<RequestT, ResponseT, ResultT>
   ...
   * operations don't stop responding. This will have to be overridden for streaming RPCs like read
   * rows.
   *
   ...
   * </ol>
   *
   * @see com.google.cloud.bigtable.grpc.io.Watchdog Watchdog which handles unresponsiveness for
   *     streaming reads.
  ...
  protected CallOptions getRpcCallOptions() {
    if (callOptions.getDeadline() != null || isStreamingRead()) {
      ...
      // If this is a streaming read, then the Watchdog will take affect and ensure that
      // unresponsiveness
      // does not occur.
