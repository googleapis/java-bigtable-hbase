...
public class BulkMutation {
  ...
  static class RequestManager {
    static boolean isNullOrEmpty(RequestManager requestManager) {
      return requestManager == null || requestManager.isEmpty();
    }
  ...
  class Batch implements Runnable {
    ...
    /**
     * An id generated by  RpcThrottler.registerRetry();
     */
    final Long retryId;

    /**
     * The current set of mutations that have not been successfully processed by the back end.
     */
    ...
    private Batch() {
      ...
      // A new Batch will always be followed with an add(), which means that a retryId is required.
      // This should be registered once, and must be unregistered at least once. RpcThrottler
      // operations should have as little synchronization around it to avoid deadlock.
      this.retryId = asyncExecutor.getRpcThrottler().registerRetry(createRetryHandler());
    }
    ...
    private synchronized void handleResult(List<MutateRowsResponse> results) {
      ...
      try {
        if (RequestManager.isNullOrEmpty(currentRequestManager)) {
    ...
    private void completeOrRetry(
      ...
      if (RequestManager.isNullOrEmpty(retryRequestManager)) {
        setRetryComplete();
      } else {
    ...
    private void setRetryComplete() {
      try {
        ...
        // This has to happen at least once, and should not be in a synchronized block in order to
        // avoid deadlocks.
     ...
     * @return the current outstanding request count.
     */
    int getRequestCount() {
      return RequestManager.isNullOrEmpty(currentRequestManager) ? 0
          : currentRequestManager.getRequestCount();
    }
  }
