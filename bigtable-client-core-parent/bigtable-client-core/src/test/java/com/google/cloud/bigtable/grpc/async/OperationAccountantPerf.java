...
public class OperationAccountantPerf {
  final static int REGISTER_COUNT = 5_000_000;
  ...
  private static void test(ListeningExecutorService pool)
    ...
    final LinkedBlockingQueue<SettableFuture<String>> registeredFutures = new LinkedBlockingQueue<>();
          ...
          public void run() {
            ...
            try {
              for (int i = 0; i < regCount; i++) {
                SettableFuture<String> future = registeredFutures.poll(1, TimeUnit.SECONDS);
                if (future == null) {
                  i--;
                } else {
                  future.set("");
