...
public class ResumingStreamingResultScannerTest {
  ...
  private void doErrorsResume(final int numExceptions)
    ...
    setupTimeoutAnswers(numExceptions, row1, row2, row3, row4);
    ResumingStreamingResultScanner scanner =
        new ResumingStreamingResultScanner(mockResponseQueueReader, mockScanHandler);

    assertRowKey("row1", scanner.next());
    assertRowKey("row2", scanner.next());
    verify(mockScanHandler, times(0)).handleTimeout(any(ScanTimeoutException.class));
    assertRowKey("row3", scanner.next());
    verify(mockScanHandler, times(numExceptions)).handleTimeout(any(ScanTimeoutException.class));
    assertRowKey("row4", scanner.next());

    scanner.close();
    verify(mockScanHandler, times(1)).cancel();
  }

  private void setupTimeoutAnswers(final int numExceptions, final FlatRow row1,
      final FlatRow row2, final FlatRow row3, final FlatRow row4)
    ...
    }).when(mockScanHandler).handleTimeout(any(ScanTimeoutException.class));
  }
}