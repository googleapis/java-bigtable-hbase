...
public class ColumnPaginationFilterAdapter extends TypedFilterAdapterBase<ColumnPaginationFilter> {
  ...
  public Filter adapt(FilterAdapterContext context, ColumnPaginationFilter filter)
    ...
    if (filter.getColumnOffset() != null) {
      ...
      return createChain(
          ...
          FILTERS
              .qualifier()
              .rangeWithinFamily(Bytes.toString(family))
              .startClosed(startQualifier));
    } else if (filter.getOffset() > 0) {
      // Include starting at an integer offset up to limit cells.
      return createChain(filter, FILTERS.offset().cellsPerRow(filter.getOffset()));
    } else {
  ...
  public FilterSupportStatus isFilterSupported(
      FilterAdapterContext context, ColumnPaginationFilter filter) {
