...
public class IpVerificationInterceptor implements ClientInterceptor {
  public static CallOptions.Key<Boolean> SKIP_IP_VERIFICATION = Key.createWithDefault("SKIP_IP_VERIFICATION", false);
        ...
        super.start(
            new SimpleForwardingClientCallListener<RespT>(responseListener) {
              ...
              public void onHeaders(Metadata headers) {
                ...
                Preconditions.checkNotNull(remoteAddr, "Call was missing TRANSPORT_ATTR_REMOTE_ADDR");

                Preconditions.checkState(
                    regex.matcher(remoteAddr.toString()).matches(),
                    "Remote address %s didn't match the pattern %s", remoteAddr, regex
                );
