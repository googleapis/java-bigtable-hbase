...
public class ComputeHashRangeFromBigtableDoFn
    extends AbstractCloudBigtableTableDoFn<RangeHash, RangeHash> {

  private static final long serialVersionUID = 1L;
  private String tableName = null;

  private transient BigtableSession session;
  private transient List<RangeHash> buffer = new LinkedList<>();
  ...
  @FinishBundle
  public void finish(FinishBundleContext context) throws IOException {
    if (buffer.isEmpty()) {
    ...
    // A happy case is that we have scanned till endKey exclusive and the last range is unverified.
    // verify it.
    ValidateBatchHash(context, resultHasher, numRows, currentRangeHash);
    // Move to next batch
    bufferIndex++;
    ...
    for (int i = bufferIndex; i < buffer.size(); i++) {
      ...
      context.output(currentRangeHash, Instant.now(), GlobalWindow.INSTANCE);
      DOFN_LOG.warn(
          "MISMATCH ON "
              + currentRangeHash.toString()
              + " . Processed "
