...
public class HBaseResultToMutationFnTest {
  ...
  /**
   * Verifies that when {@link HBaseResultToMutationFn} is called on a {@link Result} with a single
   * {@link Cell}, that cell is passed to the output, and the logger is not invoked.
   */
  @Test
  ...
  public void testResultToMutation_withDeleteMarkers() throws Exception {
    Cell[] inputCells =
        new Cell[] {
          ...
          HBaseCellUtils.deleteMarkerForOneCellWithExactTimestamp(ROW_KEY, CF, QUALIFIER, 1045),
          // Removes one cell: <CF, QUALIFIER, 1040> (4th with 0-based indexing).
          HBaseCellUtils.deleteMarkerForOneCellWithExactTimestamp(ROW_KEY, CF, QUALIFIER, 1040),
          ...
          // No effect -- nonexisting timestamp: 1045
          HBaseCellUtils.deleteMarkerForAllCellsInFamilyWithExactTimestamp(ROW_KEY, CF, 1045),
          // Removes two cells in CF2 with ts == 1040:
          // <CF2, QUALIFIER, 1040> (21st), <CF2, QUALIFIER_2, 1040> (22nd)
          HBaseCellUtils.deleteMarkerForAllCellsInFamilyWithExactTimestamp(ROW_KEY, CF2, 1040),
    ...
    assertEquals(
        Sets.newHashSet(expectedInCF),
        Sets.newHashSet(Iterables.getOnlyElement(outputs).getFamilyCellMap().get(CF)));
    assertEquals(
        Sets.newHashSet(expectedInCF2),
