...
public class HadoopHashTableSourceTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    reset(mockBigtableTableHashAccessor);
  }

  @After
  public void tearDown() throws Exception {
    verify(mockBigtableTableHashAccessor).getStartRow();
    verify(mockBigtableTableHashAccessor).getStopRow();
    verify(mockBigtableTableHashAccessor).getPartitions();
    verifyNoMoreInteractions(mockBigtableTableHashAccessor);
  ...
  public void testSplitZeroPartitions() {
    ...
    when(mockBigtableTableHashAccessor.getNumHashFiles()).thenReturn(1);
  ...
  public void testSplitOnePartition() {
    // Row range [a-z) with no splits.
    when(mockBigtableTableHashAccessor.getNumHashFiles()).thenReturn(2);
    when(mockBigtableTableHashAccessor.getPartitions()).thenReturn(ImmutableList.of(PARTITION1));
    when(mockBigtableTableHashAccessor.getStartRow()).thenReturn(START_ROW);
    when(mockBigtableTableHashAccessor.getStopRow()).thenReturn(STOP_ROW);
  ...
  public void testSplitEmptyStartRow() {
    ...
    when(mockBigtableTableHashAccessor.getPartitions())
        .thenReturn(ImmutableList.of(PARTITION1, PARTITION2));
    when(mockBigtableTableHashAccessor.getStartRow()).thenReturn(EMPTY_ROW_KEY);
    when(mockBigtableTableHashAccessor.getStopRow()).thenReturn(STOP_ROW);

    List<? extends BoundedSource<RangeHash>> actual = source.split(1, null);

    List<BoundedSource<RangeHash>> expected = ImmutableList.of(
        new KeyBasedHashTableSource(conf, HASH_TABLE_OUTPUT_PATH_DIR, EMPTY_ROW_KEY, PARTITION1),
        new KeyBasedHashTableSource(conf, HASH_TABLE_OUTPUT_PATH_DIR, PARTITION1, PARTITION2),
        new KeyBasedHashTableSource(conf, HASH_TABLE_OUTPUT_PATH_DIR, PARTITION2, STOP_ROW));
    assertEquals(expected, actual);
  }
  ...
  public void testSplitEmptyStopRow() {
    ...
    when(mockBigtableTableHashAccessor.getPartitions())
        .thenReturn(ImmutableList.of(PARTITION1, PARTITION2));
    when(mockBigtableTableHashAccessor.getStartRow()).thenReturn(START_ROW);
    when(mockBigtableTableHashAccessor.getStopRow()).thenReturn(EMPTY_ROW_KEY);
    ...
    List<BoundedSource<RangeHash>> expected = ImmutableList.of(
        new KeyBasedHashTableSource(conf, HASH_TABLE_OUTPUT_PATH_DIR, START_ROW, PARTITION1),
        new KeyBasedHashTableSource(conf, HASH_TABLE_OUTPUT_PATH_DIR, PARTITION1, PARTITION2),
        new KeyBasedHashTableSource(conf, HASH_TABLE_OUTPUT_PATH_DIR, PARTITION2, EMPTY_ROW_KEY));
    assertEquals(expected, actual);
  }
  ...
  public void testCreateKeyBasedSourceReader() throws IOException {
    ...
    when(mockBigtableTableHashAccessor.newReader(conf.get(), START_ROW)).thenReturn(mockTableHashReader);
    ...
    verify(mockBigtableTableHashAccessor).newReader(eq(conf.get()), eq(START_ROW));
  }
}