...
import com.google.cloud.bigtable.mapreduce.validation.SyncTableValidationOnlyJob.SyncMapper.Counter;
...
public class EndToEndIT {
  ...
  public void testSyncTableBigtabletoHBaseValidation() throws Exception {
    ...
    Path testSyncOutDir = HB_TEST_UTIL.getDataTestDirOnTestFS("sync-out-cbt-to-hbase-" + testRunTs);
    ...
    Counters syncCounters =
        syncTableValidationSourceBigtableTargetHBase(
            BT_TEST_SOURCE_TABLE, HB_TEST_TARGET_TABLE, testHashOutDir, testSyncOutDir);

    assertVerifySyncMatches(syncCounters, testSyncOutDir);
  }
  ...
  public void testSyncTableHBaseToBigtableValidation() throws Exception {
    ...
    Path testSyncOutDir =
        HB_TEST_UTIL.getDataTestDirOnTestFS("sync-out-hbase-to-cbt-" + +testRunTs);
    ...
    // sync table validation
    Counters syncCounters =
        syncTableValidationSourceHBaseTargetBigtable(
            HB_TEST_SOURCE_TABLE, BT_TEST_TARGET_TABLE, testHashOutDir, testSyncOutDir);

    assertVerifySyncMatches(syncCounters, testSyncOutDir);
  }

  /** assert match/mismatch results from counters and output records */
  private void assertVerifySyncMatches(Counters syncCounters, Path testSyncOutDir)
      throws IOException {
    // verify counters
    assertEquals(60, syncCounters.findCounter(Counter.ROWS_WITH_DIFFS).getValue());
    assertEquals(10, syncCounters.findCounter(Counter.SOURCE_MISSING_ROWS).getValue());
    assertEquals(10, syncCounters.findCounter(Counter.TARGET_MISSING_ROWS).getValue());
    assertEquals(50, syncCounters.findCounter(Counter.SOURCE_MISSING_CELLS).getValue());
    assertEquals(50, syncCounters.findCounter(Counter.TARGET_MISSING_CELLS).getValue());
    assertEquals(20, syncCounters.findCounter(Counter.DIFFERENT_CELL_VALUES).getValue());

    // verify output
    Map<Counter, Integer> verifyOutputMismatches = new HashMap<>();

    FileSystem fs = FileSystem.get(HB_TEST_UTIL.getConfiguration());
    JsonParser parser = new JsonParser();
    for (FileStatus file : fs.listStatus(testSyncOutDir)) {
      if (file.isFile() && file.getPath().getName().startsWith("part-m")) {
        LOG.debug("parsing output file: " + file.getPath());
        ...
        while ((line = br.readLine()) != null) {
          ...
          Counter counter =
              Counter.valueOf(json.get(RecordFormatter.REASON_CODE_STR).getAsString());
          Integer value = verifyOutputMismatches.get(counter);

          // increment counter for emitted result
          if (value == null) {
            verifyOutputMismatches.put(counter, 1);
          } else {
            verifyOutputMismatches.put(counter, value + 1);
          }
        }
        br.close();
      }
    ...
    // assert emmitted results
    assertEquals(50, verifyOutputMismatches.get(Counter.SOURCE_MISSING_CELLS).intValue());
    assertEquals(50, verifyOutputMismatches.get(Counter.TARGET_MISSING_CELLS).intValue());
    assertEquals(20, verifyOutputMismatches.get(Counter.DIFFERENT_CELL_VALUES).intValue());
  }

  /** setup config for launching with bigtable as source and hbase as target */
  private Counters syncTableValidationSourceBigtableTargetHBase(
    ...
    return syncTableValidationOnlyJob(
        sourceTableName, targetTableName, testHashOutDir, testSyncOutDir, args);
  }

  /** setup config for launching with hbase as source and bigtable as target */
  private Counters syncTableValidationSourceHBaseTargetBigtable(
      TableName sourceTableName,
      TableName targetTableName,
      Path testHashOutDir,
      Path testSyncOutDir,
  ...
  private Counters syncTableValidationOnlyJob(
      TableName sourceTableName,
      TableName targetTableName,
      Path testHashOutDir,
      Path testSyncOutDir,
    ...
    SyncTableValidationOnlyJob syncTable = new SyncTableValidationOnlyJob(conf);
    String[] args = Arrays.copyOf(options, options.length + 4);
    args[options.length] = "--sourcehashdir=" + testHashOutDir.toString();
    args[options.length + 1] = "--sourcetablename=" + sourceTableName.getNameAsString();
    args[options.length + 2] = "--targettablename=" + targetTableName.getNameAsString();
    args[options.length + 3] = "--outputpath=" + testSyncOutDir.toString();
