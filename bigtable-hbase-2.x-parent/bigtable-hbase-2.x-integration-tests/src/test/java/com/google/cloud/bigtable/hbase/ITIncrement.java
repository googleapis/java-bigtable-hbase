...
public class ITIncrement extends AbstractTest {
  @Rule public ExpectedException expectedException = ExpectedException.none();

  /**
   * Requirement 6.1 - Increment on or more columns in a given row by given amounts. Requirement 6.4
   * - Return post-increment value(s)
   */
  @Test
   ...
   * create a new value. Note: This is pretty weird. Not sure who would use it, or if we need to
   * support it. Here's the test: 1. Create a cell with three explicit versions: 101, 102, and 103.
   * The values are set to the same so we can track the original versions. 2. Call an increment (+1)
   * with a time range of [101,103) 3. It should have created a fourth version with the current
   * timestamp that incremented version 102. We can ensure this be looking at the new value (103)
   * and the number of versions. 4. Now increment (+1) with a time range outside of all versions
   * [100000, 200000) 5. It should have create a new version with the current timestamp and value of
   * 1. 6. Check all versions for the cell. You should have, in descending version order: a. A value
   * of 1 with a recent timestamp. b. A value of 103 with a recent timestamp. c. A value of 103 with
   * a timestamp of 103. d. A value of 102 with a timestamp of 102. e. A value of 101 with a
   * timestamp of 101.
   */
  @Test
  ...
  public void testIncrementWithTimerange() throws IOException {
    ...
    Put put =
        new Put(rowKey)
            .addColumn(COLUMN_FAMILY, qual, 101L, Bytes.toBytes(101L))
    ...
    Assert.assertEquals(
        "It should have incremented 102",
        102L + 1L,
    ...
    increment =
        new Increment(rowKey).addColumn(COLUMN_FAMILY, qual, 1L).setTimeRange(100000L, 200000L);
    ...
    Assert.assertTrue(
        "First should be a default timestamp", cells.get(0).getTimestamp() > 1000000000000L);
    Assert.assertEquals("Value should be new", 1L, Bytes.toLong(CellUtil.cloneValue(cells.get(0))));
    Assert.assertTrue(
        "Second should also be a default timestamp", cells.get(1).getTimestamp() > 1000000000000L);
    Assert.assertEquals(
        "Value should be an increment of 102",
        102L + 1L,
        Bytes.toLong(CellUtil.cloneValue(cells.get(1))));
    Assert.assertEquals("Third should be 103", 103L, cells.get(2).getTimestamp());
    Assert.assertEquals(
        "103 should still be 103", 103L, Bytes.toLong(CellUtil.cloneValue(cells.get(2))));
    Assert.assertEquals("Fourth should be 102", 102L, cells.get(3).getTimestamp());
    Assert.assertEquals(
        "102 should still be 101", 102L, Bytes.toLong(CellUtil.cloneValue(cells.get(3))));
    Assert.assertEquals("Last should be 101", 101L, cells.get(4).getTimestamp());
    Assert.assertEquals(
        "101 should still be 101", 101L, Bytes.toLong(CellUtil.cloneValue(cells.get(4))));
  ...
  public void testIncrementEightBytes() throws IOException {
    ...
    Result result = table.increment(increment);
    Assert.assertEquals(
        "Should have incremented the bytes like a long",
        equivalentValue + 1L,
        Bytes.toLong(CellUtil.cloneValue(result.getColumnLatestCell(COLUMN_FAMILY, qual))));
  }
  ...
  public void testIncrementWithMaxVersions() throws IOException {
    ...
    result = table.get(get);
    Assert.assertEquals(
        "Expected two results, only the latest version for each column", 2, result.size());
    Assert.assertEquals(
        "Value1=" + value1 + " & Incr1=" + incr1,
        value1 + incr1,
        Bytes.toLong(CellUtil.cloneValue(result.getColumnLatestCell(incrementFamily, qual1))));
