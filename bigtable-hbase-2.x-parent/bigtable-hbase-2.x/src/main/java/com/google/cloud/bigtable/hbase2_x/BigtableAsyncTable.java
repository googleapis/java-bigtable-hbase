...
public abstract class BigtableAsyncTable implements AsyncTable<ScanResultConsumer> {
   ...
   * class that extends BigtableAsyncTable so incompatible methods won't be accessed unless the
   * methods are called. If a method is implemented by BigtableAsyncTable, the generated class will
   * invoke the implementation in BigtableAsyncTable. Otherwise it'll throw {@link
