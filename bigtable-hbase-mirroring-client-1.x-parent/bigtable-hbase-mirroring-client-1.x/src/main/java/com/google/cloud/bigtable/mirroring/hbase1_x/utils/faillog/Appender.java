...
public class Appender implements AutoCloseable {
   ...
   * Create an `Appender`.
   *
   * <p>The created `Appender` will create a thread for flushing the data asynchronously. The data
   * will be transferred to the thread via a buffer of a given maximum size (`maxBufferSize`).
   * Entries larger than `maxBufferSize` will be accepted by the `Appender`, breaking the
