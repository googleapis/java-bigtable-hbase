...
public class TestBatchExecutor {
  ...
  public void testGet() throws Exception {
    final FlatRow response = FlatRow.newBuilder().build();
  ...
  public void testGetCallback() throws Exception {
    FlatRow response = FlatRow.newBuilder().build();
    when(mockFuture.get()).thenReturn(ImmutableList.of(response));
  ...
  public void testBatchBulkGets() throws Exception {
    ...
    for (int i = 1; i < 10; i++) {
      gets.add(new Get(Bytes.toBytes("key" + i)));
    }
    ResultScanner<FlatRow> mockScanner = Mockito.mock(ResultScanner.class);
    when(mockClient.readFlatRows(any(ReadRowsRequest.class))).thenReturn(mockScanner);
    final AtomicInteger counter = new AtomicInteger();
    when(mockScanner.next()).then(new Answer<FlatRow>() {
      @Override
      public FlatRow answer(InvocationOnMock invocation) throws Throwable {
        ...
        final FlatRow row = FlatRow.newBuilder().withRowKey(key)
    ...
    for (int i = 1; i < results.length; i++) {
      Assert.assertTrue(
        String.format("%s and %s are not equal for result %s", 
          Bytes.toString(results[i].getRow()),
          Bytes.toString(gets.get(i).getRow()), i),
