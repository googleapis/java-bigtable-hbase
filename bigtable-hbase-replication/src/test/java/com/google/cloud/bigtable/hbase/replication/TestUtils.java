...
public class TestUtils {
  ...
  public static void assertEquals(String message, byte[] expected, byte[] actual) {
    Assert.assertEquals(message, new String(expected, StandardCharsets.UTF_8),
  ...
  public static void assertEquals(Cell expected, Cell actual) {
    ...
    assertEquals("Column qualifier mismatch for row " + rowKey, CellUtil.cloneQualifier(expected),
        CellUtil.cloneQualifier(actual));
    Assert.assertEquals("Timestamp mismatch for row " + rowKey, expected.getTimestamp(),
        actual.getTimestamp());
    assertEquals("Value mismatch for row " + rowKey, CellUtil.cloneRow(expected),
        CellUtil.cloneRow(actual));
  }
  ...
  public static void assertEquals(Result expected, Result actual) {
    ...
    Assert.assertEquals("Result cell count mismatch for row " + new String(expected.getRow()) + "\n \t\texpected: " + expectedCells + "\n \t\tactual: " + actualCells, expected.size(), actual.size());
  ...
  public static void assertTableEquals(Table expected, Table actual) throws IOException {
    ...
    while (expectedIterator.hasNext()) {
      ...
      if (!actualIterator.hasNext()) {
        Assert.fail("Expected less rows. Extra results starting at " + Bytes
            .toStringBinary(expectedIterator.next().getRow()));
      }
    ...
    if (actualIterator.hasNext()) {
      Assert.fail("Expected more rows, missing rows starting from " + Bytes
          .toStringBinary(actualIterator.next().getRow()));
    }
  }
}
