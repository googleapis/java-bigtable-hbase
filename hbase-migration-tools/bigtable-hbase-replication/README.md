...
3. Update hbase-site.xml with destination cloud bigtable project and instance
...
9. Eventually, replication will catch up and the 2 databases will be in sync. On
   HBase shell execute `status 'replciation'` to check status of replication for
...
Have the below properties set in `hbase-site.xml` and add it to the HBase region
server classpath.
...
for other properties that can be set. Please set other properties for
authenticating to Cloud Bigtable based on
your preferred [authentication](https://cloud.google.com/bigtable/docs/authentication)
method.
...
1. Install Cloud Bigtable replication library in HBase servers (both master and
   region servers).
    1. Download the library from maven on all the master and region server
       nodes.
    2. Add the library to the HBase region server class path. In DataProc, this
       can be achieved by copying the library to /usr/lib/hbase/lib/
2. Add CBT configs to hbase-site.xml
3. Configure Cloud Bigtable authentication as
   shown [here](https://cloud.google.com/bigtable/docs/authentication).
4. Restart the HBase master, this allows the master to load the replication jar
   and be aware of the classes in it.
...
Another special case is mutations with reverse timestamps. HBase uses a `long`
to store milliseconds while Cloud Bigtable uses `long` to store microseconds.
This difference in granularity means, HBase can store 1000 times higher
timestamp than Cloud Bigtable. The impacted use case is the reverse timestamp,
where customers use `Long.MAX_VALUE - now` as the cell timestamp. Such
timestamps may get truncated in Cloud Bigtable.
